...
public class LinkedListImp<E>
   ...
   */
  public LinkedListImp(Collection<? extends E> c) {
    this();
    addAll(c);
  }
   ...
   * with the lowest index {@code i} such that {@code Objects.equals(o, get(i))} (if such an element
   * exists).  Returns {@code true} if this list contained the specified element (or equivalently,
   ...
   * specified position in the list. Obeys the general contract of {@code
   * List.listIterator(int)}.<p>
   * <p>
   * The list-iterator is <i>fail-fast</i>: if the list is structurally modified at any time after
   * the Iterator is created, in any way except through the list-iterator's own {@code remove} or
   * {@code add} methods, the list-iterator will throw a {@code ConcurrentModificationException}.
  ...
  private LinkedListImp<E> superClone() {
    try {
      return (LinkedListImp<E>) super.clone();
   ...
   */
  public Object clone() {
    LinkedListImp<E> clone = superClone();
  ...
  static final class LLSpliterator<E> implements Spliterator<E> {
    ...
    final LinkedListImp<E> list; // null OK unless traversed
    Node<E> current;      // current node; null until initialized
    int est;              // size estimate; -1 until first needed
    int expectedModCount; // initialized when est set
    int batch;            // batch size for splits

    LLSpliterator(LinkedListImp<E> list, int est, int expectedModCount) {
      this.list = list;
      this.est = est;
